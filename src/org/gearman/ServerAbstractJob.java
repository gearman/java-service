...
abstract class ServerAbstractJob implements ServerJob, ServerClientDisconnectListener {
	...
	public static final ServerJob getJob(final ByteArray jobHandle) {
		return ServerAbstractJob.globalJobs.get(jobHandle);
	}
	...
	ServerAbstractJob(final ByteArray uniqueID, final byte[] data, final JobPriority priority, final ServerClient creator) {
		...
		ServerAbstractJob.globalJobs.put(this.jobHandle, this);
	}
	...
	public final GearmanPacket createJobAssignPacket() {
		return GearmanPacket.createJOB_ASSIGN(jobHandle.getBytes(), this.getFunction().getName().toString(GearmanVariables.UTF_8), data);
	}
	...
	public void workComplete(GearmanPacket packet) {
		...
		ServerAbstractJob.globalJobs.remove(this.jobHandle);
	}
	...
	private static final byte[] initJobHandle() {
		byte[] user;
		try {
			user = java.net.InetAddress.getLocalHost().getHostName().getBytes(GearmanVariables.UTF_8);
	...
	private synchronized static final byte[] getNextJobHandle() {
		final byte[] jobNumber = Long.toString(jobHandleNumber).getBytes(GearmanVariables.UTF_8);
